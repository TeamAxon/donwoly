...
def test_embed_query_uses_confirmed_model_and_returns_1536_vector(monkeypatch):
    ...
    vector = qdrant_search.embed_query("Australian minimum wage")

    assert captured == {
        "model": "text-embedding-3-small",
        "input": "Australian minimum wage",
    }
...
def test_search_chunks_applies_category_filter_and_maps_payload(monkeypatch):
    ...
    result = qdrant_search.search_chunks(
        "Australian minimum wage", category="labor", top_k=3
    )
